//
//  GS1SyntaxDictionaryData.swift
//
//  The GS1 Barcode Syntax Dictionary, embedded verbatim so the package has no
//  runtime dependencies and no data files to ship.
//
//  Source: https://github.com/gs1/gs1-syntax-dictionary
//  Copyright (c) 2020-2021 BWIPP project, 2020-2021 Zint Project,
//  2021-2025 GS1 AISBL. Licensed under the Apache License, Version 2.0.
//
//  To update: replace the string below with the data lines of a newer
//  release of gs1-syntax-dictionary.txt. Comments and blank lines are ignored,
//  so the file can be pasted in unmodified.
//

public enum GS1SyntaxDictionary {

    /// Parsed table, keyed by AI.
    public static let table: [String: GS1ApplicationIdentifier] = {
        var result: [String: GS1ApplicationIdentifier] = [:]
        for line in rawText.split(separator: "\n", omittingEmptySubsequences: true) {
            let text = String(line).trimmed()
            if text.isEmpty || text.hasPrefix("#") { continue }
            // A malformed embedded table is a programming error, not a runtime
            // condition, so failures are surfaced immediately.
            guard let definitions = try? GS1ApplicationIdentifier.parse(dictionaryLine: text) else {
                preconditionFailure("Embedded GS1 syntax dictionary line failed to parse: \(text)")
            }
            for definition in definitions {
                result[definition.ai] = definition
            }
        }
        return result
    }()

    /// All known AIs, sorted.
    public static let allAIs: [String] = table.keys.sorted()

    public static func definition(for ai: String) -> GS1ApplicationIdentifier? {
        table[ai]
    }

    /// Two-digit prefixes whose AIs have a pre-defined element string length.
    /// Derived from the table, and used to decide whether an FNC1 separator is
    /// required after a field.
    public static let predefinedLengthPrefixes: Set<String> = {
        var prefixes: Set<String> = []
        for (ai, definition) in table where definition.predefinedLength {
            prefixes.insert(String(ai.prefix(2)))
        }
        return prefixes
    }()
    ...
    243 ? X..20 req=01 # PCN
    250 ? X..30 req=01+21,8006+21 # SECONDARY SERIAL
    ...
    3140-3145 *? N6 req=01,02 ex=314n # AREA (m2)
    3150-3155 *? N6 req=01,02 ex=315n # NET VOLUME (l)
    3160-3165 *? N6 req=01,02 ex=316n # NET VOLUME (m3)
    ...
    3340-3345 *? N6 req=00,01 ex=334n # AREA (m2), log
    3350-3355 *? N6 req=00,01 ex=335n # VOLUME (l), log
    3360-3365 *? N6 req=00,01 ex=336n # VOLUME (m3), log
    3370-3375 *? N6 req=01 ex=337n # KG PER m2
    ...
    3500-3505 *? N6 req=01,02 ex=350n # AREA (in2)
    3510-3515 *? N6 req=01,02 ex=351n # AREA (ft2)
    3520-3525 *? N6 req=01,02 ex=352n # AREA (yd2)
    3530-3535 *? N6 req=00,01 ex=353n # AREA (in2), log
    3540-3545 *? N6 req=00,01 ex=354n # AREA (ft2), log
    3550-3555 *? N6 req=00,01 ex=355n # AREA (yd2), log
    ...
    3640-3645 *? N6 req=01,02 ex=364n # NET VOLUME (in3)
    3650-3655 *? N6 req=01,02 ex=365n # NET VOLUME (ft3)
    3660-3665 *? N6 req=01,02 ex=366n # NET VOLUME (yd3)
    3670-3675 *? N6 req=00,01 ex=367n # VOLUME (in3), log
    3680-3685 *? N6 req=00,01 ex=368n # VOLUME (ft3), log
    3690-3695 *? N6 req=00,01 ex=369n # VOLUME (yd3), log
    ...
    427 ? X..3 req=01+422,02+422 # ORIGIN SUBDIVISION
    ...
    7004 ? N..4 req=01+10 # ACTIVE POTENCY
    ...
    7020 ? X..20 req=01+416,8006+416 # REFURB LOT
    ...
    8112 ? X..70,couponposoffer # PAPERLESS COUPON
